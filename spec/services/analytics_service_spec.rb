...
RSpec.describe AnalyticsService, type: :service do
  ...
  describe '.record_search' do
    context 'when USE_REDIS_FOR_ANALYTICS is true' do
      ...
      it 'calls record_in_redis for non-empty query' do
      ...
      context 'when the query is empty' do
        let(:empty_query) { '' }
        it 'does nothing' do
          expect(AnalyticsService).not_to receive(:record_in_redis)
          AnalyticsService.record_search(empty_query, ip)
        end
      end
    end
    ...
    context 'when USE_REDIS_FOR_ANALYTICS is false' do
      ...
      it 'enqueues FindRecordsJob for non-empty query' do
      ...
      context 'when the query is empty' do
        let(:empty_query) { '' }
        it 'does not enqueue FindRecordsJob' do
          expect(FindRecordsJob).not_to receive(:perform_later)
          AnalyticsService.record_search(empty_query, ip)
        end
      end
    end
  ...
  describe '.record_in_redis' do
    ...
    context 'when the query contains stop words' do
      ...
      it 'skips stop words and stores only allowed words' do
        expect(redis_instance).to receive(:incr).with("analytics:search_query:#{query}:count")
        expect(redis_instance).to receive(:set).with("analytics:search_log:#{query}:ip", ip)
  ...
  describe 'database changes when using the background job' do
    ...
    it 'creates a SearchQuery and SearchLog record with transformed content for non-empty query' do
      expect do
        AnalyticsService.record_search(query, ip)
      ...
      expect(search_log.words).to match_array(%w[hello world])
      expect(search_log.ip).to eq(ip)
    end

    context 'when the query is empty' do
      let(:empty_query) { '' }
      it 'does not create any SearchQuery or SearchLog records' do
        expect do
          AnalyticsService.record_search(empty_query, ip)
        end.not_to(change { SearchQuery.count })
      end
    end
  end
end
